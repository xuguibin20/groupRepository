...
export default {
  ...
  watch: {
    option (opt) {
      ...
      this.coption = opt
      this.ui.setMultiSelect(opt.multiSelect)
    },
    data (val) {
  ...
  mounted () {
    window.console.log('combobox mounted.' + this.cid + this.idhash)
    var dom = this.$refs.dom
    if (this.ui == null) {
      // eslint-disable-next-line new-cap
      this.ui = new window.qzz.ui.input.combobox(dom, this.coption)
  ...
  destroyed() {
    window.console.info('destroy combobox')
