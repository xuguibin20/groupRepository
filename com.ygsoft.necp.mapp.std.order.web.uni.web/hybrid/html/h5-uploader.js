	...
	xhr.onreadystatechange = (ev) => {
		...
		if(xhr.readyState == 4) {
			...
			if (xhr.status == 200) {
				...
				location.href = `callback?status=${escape(xhr.status)}&fileName=${escape(file.name)}&response=${escape(xhr.responseText)}`;
			}
			else {
				progress.innerText = '上传失败了';
				location.href = `callback?status=${escape(xhr.status)}`;
			}
...
document.addEventListener('UniAppJSBridgeReady', () => {
	...
	fileDom.addEventListener('change', (event) => {
		...
		if(file.size > (1024*1024 * 10)) {
			plus.nativeUI.toast('上传文件大小不能超过10MB！');
			return;
		}
